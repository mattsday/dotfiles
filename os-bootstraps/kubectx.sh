...
if [ -z "${KUBECTX_FORCE}" ] && command -v kubectx >/dev/null 2>&1 && command -v kubens >/dev/null 2>&1; then
    echo Kubectx and Kubens already installed, set KUBECTX_FORCE=1 to force
    exit 0
fi
...
download() {
    ...
    if [ -f "${downloaded_file}" ]; then
        rm "${downloaded_file}" || fail "${downloaded_file}" exists and could not be deleted
    fi
    curl -sL --fail "${url}" -o "${downloaded_file}" || fail Download of "${url}" failed
    if [ ! -f "${downloaded_file}" ]; then
        fail "Download failed (not found in ${downloaded_file})"
    fi
    tar zxvf "${downloaded_file}" -C "${TMP_DIR}" >/dev/null 2>&1 || fail Could not extract "${downloaded_file}"
    if [ -f "${TMP_DIR}/${output_file}" ]; then
        sudo mv "${TMP_DIR}/${output_file}" "${KUBECTX_PREFIX}"
    else
        fail Could not download "${output_file}"
    fi
}
